-- Fix check_agent_permission to use correct column name (rate_limit_per_hour, not rate_limit_per_minute)
CREATE OR REPLACE FUNCTION public.check_agent_permission(p_agent_name text, p_operation text, p_target_table text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Fix get_agent_stats to use correct column name
