...
-- Create gateway functions in the api schema (which PostgREST exposes)
...
CREATE OR REPLACE FUNCTION api.check_agent_permission(
  p_agent_name text,
  p_operation text,
  p_target_table text
) RETURNS boolean
...
CREATE OR REPLACE FUNCTION api.log_agent_operation(
  p_agent_name text,
  p_operation text,
  p_target_table text,
